...
const skillsData = [
  ...
  { title: 'Databases', tags: ['SQL', 'MongoDB'] },
  {
    ...
    tags: ['OpenCV', 'TensorFlow', 'Pandas', 'NumPy', 'GitHub'],
  },
];
...
const projects = [
  {
    ...
    text: 'Full-stack web app for browsing, filtering, and booking travel packages with accounts and SQL-backed bookings.',
  ...
  },
  {
    ...
    text: 'Real-time distress detection with OpenCV and TensorFlow, integrated with Telegram for instant alerts.',
  ...
  },
  {
    ...
    text: 'Improved forecast accuracy by ~25% using feature engineering and ML, visualized with Power BI.',
  ...
  },
  {
    ...
    text: 'Reservation and schedule management system for hall bookings, customer records, and automated availability.',
    stack: ['HTML', 'CSS', 'JavaScript', 'SQL'],
  },
];
...
const experienceItems = [
  ...
  },
  {
    ...
    date: '2024',
    text: 'Delivered a national-level prototype under a real hackathon problem statement.',
  },
  {
    title: 'TANSAM Hackathon 2025 — State Level',
    date: '2025',
    text: 'Built a working prototype under a tight deadline during state-level competition.',
  },
];
...
function App() {
  ...
  useEffect(() => {
    ...
    const onScroll = () => {
      nav.style.boxShadow =
        window.scrollY > 40 ? '0 8px 30px rgba(0,0,0,.35)' : 'none';
    };
  ...
  return (
    <div className="wrap">
      ...
      <Experience />
      <Contact />
      <footer>© 2026 Abirami · Built with code, coffee, and real-world projects.</footer>
    </div>
  );
}
  ...
  return (
    <nav>
      ...
      <div style={{ display: 'flex', gap: 12, alignItems: 'center' }}>
        <button className="nav-cta" onClick={onToggleTheme}>
      ...
      <div className="hero-grid">
        <div>
          ...
          <p className="hero-sub reveal">
            B.Tech Information Technology graduate (CGPA 8.1) from A.V.C College of Engineering, with hands-on AI/ML internship experience and a track record of shipping real, working projects.
          </p>
...
function About() {
  return (
    <Section
      id="about"
      tag="// 01 about"
      title="Fresher on paper, builder in practice."
      description="I learn by shipping. Every line on this page is backed by a working repo, a deployed demo, or a real internship deliverable."
    >
...
function Projects({ selectedProject, setSelectedProject }) {
  ...
  return (
    <Section
      id="projects"
      tag="// 03 work"
      title="Projects with a real problem behind them."
      description="Each one solves something specific — safety, environment, travel, or analytics — end to end."
    >
      <div className="proj-grid">
        {projects.map((project, index) => (
          ...
          >
            ...
            <h3>{project.title}</h3>
            <p>{project.text}</p>
            <div className="proj-stack">{project.stack.map((stack) => <span key={stack}>{stack}</span>)}</div>
          </div>
        ))}
      </div>
      <div className="status-card reveal" style={{ marginTop: '2rem' }}>
        ...
        <div className="status-body">
          ...
          <p style={{ marginTop: 14, color: 'var(--muted)' }}>Click a card to highlight the project and keep your portfolio experience interactive.</p>
        </div>
      </div>
    </Section>
  );
...
function Certifications() {
  return (
    <Section id="certifications" tag="// 05 certificates" title="Certifications that back my skillset.">
...
function Resume() {
  return (
    <Section id="resume" tag="// 06 resume" title="Resume and professional profile.">
      <div className="resume-preview reveal">
        <div className="resume-summary">
          <h2>Professional Profile</h2>
          <p>AI and full-stack developer focused on building reliable systems for safety, travel, and analytics.</p>
        </div>
        <a className="btn btn-primary" href="assets/ABI_CV_2026.pdf" download>Download Resume</a>
      </div>
    </Section>
  );
}
